...
class StoreNormalIn extends StatefulWidget {
  final List<HardwareStatusData> hardwareStatusData;
  final Function(List<HardwareStatusData>) onHardwareStatusDataChanged;

  const StoreNormalIn({Key? key, required this.hardwareStatusData, required this.onHardwareStatusDataChanged}) : super(key: key);

  @override
  _StoreNormalInState createState() => _StoreNormalInState();
}

class _StoreNormalInState extends State<StoreNormalIn> {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.hardwareStatusData.length; i++) {
      ...
      _locations[data.id] = 'A'; // Location par défaut pour les produits "Normal"
                  ...
                  cells: [
                    ...
                    DataCell(
                      TextFormField(
                        ...
                        enabled: true, // Désactivez la modification si le statut est "Faulty"
                      ),
                    ),
                    DataCell(
                      Text(data.status),
